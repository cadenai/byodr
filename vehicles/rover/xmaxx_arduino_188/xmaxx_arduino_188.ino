...
// For odometry.
volatile byte h_rotation;
volatile float h_rps;
volatile uint32_t h_prev_time, h_detect_time;
...
void hall_detect() {
  if (digitalRead(HALL_IN_PIN) == HIGH) {
    ...
    if (h_rotation > 1) {
      // Time spent in two revolutions - convert to revolutions per second.
      h_rps = 2000000.0 / (micros() - h_prev_time);
      h_prev_time = micros();
      h_rotation = 0;
    }
  }
